        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                if isinstance(model_output, tuple):
                    interleave_loss, additional_loss_dict = model_output
                else:
                    interleave_loss = model_output
            ...
            metric_logger.update(interleave_loss=loss_value)
            
            if additional_loss_dict:
                for key, val in additional_loss_dict.items():
                    if isinstance(val, torch.Tensor):
                        val = val.detach().float()
                    metric_logger.update(**{key: val.item() if isinstance(val, torch.Tensor) else val})
